(function ($) {
  ...
  $(document).ready(function () {
    ...
    $(".wpentrepreneur-remove-notice").on("click", function (event) {
      ...
      $.ajax({
        ...
        success: function () {
          $notice.closest(".wpentrepreneur-notice").fadeOut();
        },
      });
    });
    ...
    let $recommendedPlugins = $(
      ".wpentrepreneur-dashboard-wrapper .starter-template-action a"
    );
    if ($recommendedPlugins) {
      let $pluginInfo = $(
          ".wpentrepreneur-dashboard-wrapper .starter-template-plugins-info"
        ),
